...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/qwen3_vl_sft_lora}"
TRAIN_JSONL="${TRAIN_JSONL:-data/sft_v2/sft_train.jsonl}"
EVAL_JSONL="${EVAL_JSONL:-data/sft_v2/sft_val.jsonl}"
DATA_ROOT="${DATA_ROOT:-data/sft_v2}"
...
if [[ "${FREEZE_VISION_LORA:-0}" == "1" ]]; then
  TRAIN_ARGS+=(--freeze-vision-lora)
else
  TRAIN_ARGS+=(--no-freeze-vision-lora)
fi
